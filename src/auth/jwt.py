...
def decode_jwt(
    token: str, public_key: str = settings.jwt.public_key_path.read_text()
) -> dict:
    decoded: dict = jwt.decode(token, public_key, algorithms=[settings.jwt.ALGORITHM])
    return decoded
...
def encode_jwt(
    ...
    encoded: str = jwt.encode(payload, private_key, algorithm=settings.jwt.ALGORITHM)
    return encoded


def create_access_token(
    payload: dict,
    access_token_expire_time: timedelta = timedelta(
...
def create_refresh_token(
    payload: dict,
    refresh_token_expire_time: timedelta = timedelta(
